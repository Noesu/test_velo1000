...
def test_search_query(page, case):
    query = case["query"]
    ...
    with allure.step(f"{description}. Query:{query}"):
        ...
        page.search_engine.insert_search(query)
        try:
            search_results_page = page.search_engine.click_submit_search_button()
        except Exception:
            allure.attach(
        ...
        if not expect_navigation:
            assert page.url_changed_from_base() == False, "Url changed after submitting empty request"
