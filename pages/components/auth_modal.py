...
class AuthModalComponent:
    ...
    # 1.2 Authorization tab element locators
    ...
    AUTH_REMEMBER_ME_INPUT = (By.CSS_SELECTOR, 'input#remember')
    AUTH_REMEMBER_ME_LABEL = (By.CSS_SELECTOR, 'label.input-checkbox-circle')
    ...
    # 1.3 Registration tab element locators
    ...
    # 2.2 Main state checkers
    def is_modal_window_not_visible(self, timeout: Union[int, float]):
        try:
    ...
    # 3.1 Authorization tab element getters:
    ...
    # 3.2 Authorization tab state checkers
    ...
    def is_auth_password_visible(self) -> bool:
    ...
    def is_auth_password_visibility_btn_present(self) -> bool:
        try:
    ...
    def is_auth_remember_me_checkbox_clickable(self) -> bool:
        try:
            self.wait.until(EC.element_to_be_clickable(self.AUTH_REMEMBER_ME_LABEL))
            return True
        except TimeoutException:
            return False

    def is_remember_me_checkbox_checked(self) -> Optional[bool]:
        try:
    ...
    def set_auth_remember_me_checkbox(self, switch_on: bool) -> Optional[bool]:
        try:
            checkbox: WebElement = self.wait.until(EC.presence_of_element_located(self.AUTH_REMEMBER_ME_INPUT))
            if checkbox.is_selected() != switch_on:
                ...
                self.wait.until(EC.element_located_selection_state_to_be(self.AUTH_REMEMBER_ME_INPUT, switch_on))
            return self.is_remember_me_checkbox_checked()
        except Exception:
            return None
    ...
    # 4.1 Registration tab element getters:
    ...
    # 4.2 Registration tab state checkers:
